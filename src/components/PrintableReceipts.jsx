...
const PrintableReceipts = forwardRef(({ receipts, user }, ref) => {
  return (
    // Attach the forwarded ref to the root DOM element you want to print.
    // This is the key change that resolves the printing error.
    <div ref={ref} className="p-8 font-sans">
      <h1 className="text-3xl font-bold mb-4">Voting Receipts for {user.firstName} {user.lastName}</h1>
        ...
        <div className="space-y-4">
          {receipts.map((receipt) => (
            <div key={receipt.id} className="border border-gray-300 p-4 rounded-lg">
              ...
              <p className="text-sm text-gray-500">Voted on: {new Date(receipt.timestamp).toLocaleString()}</p>
            </div>
          ))}
        </div>
      )}
